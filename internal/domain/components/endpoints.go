...
type Endpoints struct {
	...
	RequestBody    *map[string]interface{} `json:"request_body" db:"request_body"`
...
type ValidationEndpoints struct {
	...
	RequestBody    *map[string]interface{} `json:"request_body" binding:"omitempty"`
}

type InterfaceEndpoints interface {
	CreatedEndpoints(ctx context.Context, endp *Endpoints) error
	//EnpointBysubresource(ctx context.Context, subr int *[])
}
